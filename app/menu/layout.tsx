...
export default async function MenuLayout({ children }: Props) {
	...
	return (
		<div className='wrapper flex space-x-8 py-16 px-4'>
			<div className='max-lg:hidden'>
				<ol className='menu-item'>
					<li className='menu-item-header'>Drinks</li>
					{drinks.map(drink => (
						<Link key={drink.id} href={`${drink.slug}`}>
							<li>{drink.name}</li>
						</Link>
					))}
				</ol>
				<ol className='menu-item'>
					...
					{foods.map(food => (
						<Link key={food.id} href={`${food.slug}`}>
							<li>{food.name}</li>
						</Link>
					))}
				</ol>
				<ol className='menu-item'>
					...
					{homes.map(home => (
						<Link key={home.id} href={`${home.slug}`}>
							<li>{home.name}</li>
						</Link>
					))}
				</ol>
				<ol className='menu-item'>
					...
					{merchs.map(merch => (
						<Link key={merch.id} href={`${merch.slug}`}>
							<li>{merch.name}</li>
						</Link>
					))}
				</ol>
